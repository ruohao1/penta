...
func newStdoutReporter(out io.Writer, verbosity Verbosity, color bool, redactReport bool) *stdoutReporter {
	return &stdoutReporter{out: out, verbosity: verbosity, startedAt: time.Now(), printedPhases: map[string]bool{}, styles: newCLIStyles(color), color: color, redactReport: redactReport}
}
...
func (r *stdoutReporter) RunCompleted(summary *viewmodel.RunSummary) {
	...
	fprintf(r.out, "%s", reporting.RenderTerminalReportWithOptions(summary, reporting.RenderOptions{Redact: r.redactReport, Color: r.color}))
}
